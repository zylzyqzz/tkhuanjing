...
const platform=(path:string,name:string,title:string)=>({path:`/platform/${path}`,name,component:path==='overview'?PlatformDashboardView:path==='organizations'?PlatformView:path==='subscriptions'?PlanFeaturesView:path==='features'?FeatureCenterView:path==='organization-features'?OrganizationFeaturesView:path==='devices'?DeviceFeaturesView:path==='roles'?RoleMatrixView:PlaceholderView,meta:{audience:'platform',title}})
...
export const router=createRouter({history:createWebHistory('/tk-admin/'),routes:[
 ...
 platform('overview','platform-overview','平台总览'),platform('organizations','platform-organizations','企业管理'),platform('subscriptions','platform-subscriptions','订阅与套餐'),platform('features','platform-features','功能模块'),platform('organization-features','platform-organization-features','企业功能授权'),platform('devices','platform-devices','设备功能灰度'),platform('roles','platform-roles','角色权限矩阵'),platform('providers','platform-providers','Provider'),platform('releases','platform-releases','版本发布'),platform('settings','platform-settings','系统配置'),platform('alerts','platform-alerts','平台告警'),platform('audit','platform-audit','平台审计'),
 ...
 {path:'/enterprise/devices/:deviceId',name:'enterprise-device-detail',component:DeviceDetailView,meta:{audience:'enterprise',title:'设备详情',feature:'device_center',permission:'devices.read'}},
 ...
 {path:'/platform',redirect:'/platform/overview'},{path:'/dashboard',redirect:'/enterprise/dashboard'},{path:'/realtime-devices',redirect:'/enterprise/devices'},{path:'/devices',redirect:'/enterprise/devices'},{path:'/accounts',redirect:'/enterprise/live'},{path:'/reports',redirect:'/enterprise/analytics'},{path:'/alerts',redirect:'/enterprise/alerts'},{path:'/members',redirect:'/enterprise/organization'},{path:'/:pathMatch(.*)*',name:'not-found',component:AccessStateView,meta:{title:'页面不存在'}}
]})
