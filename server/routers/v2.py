...
from ..models import Admin,AuditLog,CheckReport,Customer,Device,LiveRoom,now_iso
...
from ..schemas_v2 import AccountCreateIn,AnchorCreateIn,BindingDeleteIn,BindingPutIn,FeatureAssignmentIn,FeatureAssignmentsIn,FeatureDefinitionIn,FeatureRolloutIn,HeartbeatIn,MemberCreateIn,MemberLoginIn,MemberRoleUpdateIn,OnboardingProgressIn,OrganizationProfileIn,RoleFeaturePermissionsIn,RoomCreateIn
...
def binding_detail_view(db:Session,x:DeviceRoomBinding)->dict:
 value=binding_view(x);room=db.get(LiveRoom,x.room_id);account=db.get(LiveAccount,x.account_id) if x.account_id else None;anchor=db.get(AnchorProfile,x.anchor_id) if x.anchor_id else None
 return {**value,"room_name":room.name if room else "","account_name":account.display_name if account else "","anchor_name":anchor.display_name if anchor else ""}
...
def create_organization_member(payload:MemberCreateIn,ctx:TenantContext=Depends(current_member),db:Session=Depends(get_db))->dict:
 authorize_feature_action(ctx,db,"member_management","manage","members.manage")
 if db.scalar(select(OrganizationMember).where(OrganizationMember.organization_id==ctx.organization_id,OrganizationMember.username==payload.username)):
  raise api_error("MEMBER_EXISTS","该企业成员账号已存在")
 row=OrganizationMember(organization_id=ctx.organization_id,username=payload.username,password_hash=hasher.hash(payload.password),display_name=payload.display_name,role=payload.role)
 db.add(row);db.flush();audit_v2(db,ctx,"create_organization_member","organization_member",str(row.id),json.dumps({"username":row.username,"role":row.role},ensure_ascii=False));db.commit()
 return {"id":row.id,"username":row.username,"display_name":row.display_name,"role":row.role,"active":row.active,"created_at":row.created_at}
...
def get_binding(device:Device=Depends(current_v2_device),db:Session=Depends(get_db))->dict:
 ...
 return {"binding":binding_detail_view(db,row) if row else None,"synced_at":datetime.now(timezone.utc)}
...
@router.get("/organization/devices/{device_id}")
def organization_device_detail(device_id:str,ctx:TenantContext=Depends(current_member),db:Session=Depends(get_db))->dict:
 require_permission(ctx,"devices.read")
 device=db.scalar(select(Device).where(Device.device_id==device_id,Device.customer_id==ctx.organization_id))
 if not device:raise api_error("TENANT_RESOURCE_NOT_FOUND","设备不存在或属于其他企业")
 history=db.scalars(select(DeviceRoomBinding).where(DeviceRoomBinding.organization_id==ctx.organization_id,DeviceRoomBinding.device_id==device_id).order_by(DeviceRoomBinding.id.desc()).limit(50)).all()
 room_ids={x.room_id for x in history};account_ids={x.account_id for x in history if x.account_id};anchor_ids={x.anchor_id for x in history if x.anchor_id}
 rooms={x.id:x.name for x in db.scalars(select(LiveRoom).where(LiveRoom.customer_id==ctx.organization_id,LiveRoom.id.in_(room_ids))).all()} if room_ids else {}
 accounts={x.id:x.display_name for x in db.scalars(select(LiveAccount).where(LiveAccount.organization_id==ctx.organization_id,LiveAccount.id.in_(account_ids))).all()} if account_ids else {}
 anchors={x.id:x.display_name for x in db.scalars(select(AnchorProfile).where(AnchorProfile.organization_id==ctx.organization_id,AnchorProfile.id.in_(anchor_ids))).all()} if anchor_ids else {}
 cutoff=datetime.now(timezone.utc)-timedelta(minutes=60)
 metrics=db.scalars(select(DeviceHeartbeatV2).where(DeviceHeartbeatV2.organization_id==ctx.organization_id,DeviceHeartbeatV2.device_id==device_id,DeviceHeartbeatV2.received_at>=cutoff).order_by(DeviceHeartbeatV2.received_at).limit(720)).all()
 logs=db.scalars(select(AuditLog).where(AuditLog.actor.like(f"org:{ctx.organization_id}:%"),or_(AuditLog.target_id==device_id,AuditLog.details.contains(device_id))).order_by(AuditLog.id.desc()).limit(50)).all()
 return {"device":{"device_id":device.device_id,"display_name":device.display_name or device.device_id,"status":device.status,"agent_version":device.app_version,"online_state":device.online_state,"studio_state":device.studio_state,"collector_state":device.collector_state,"last_heartbeat_at":device.last_heartbeat_at,"store_name":device.store_name,"location":device.location,"owner_name":device.owner_name,"target_region_id":device.target_region_id,"metrics":{"cpu_percent":device.cpu_percent,"memory_percent":device.memory_percent,"network_latency_ms":device.network_latency_ms,"upload_mbps":device.upload_mbps,"stream_bitrate_kbps":device.stream_bitrate_kbps,"dropped_frames":device.dropped_frames}},"binding":binding_view(next((x for x in history if x.status=="active" and x.binding_type=="primary"),None)) if any(x.status=="active" and x.binding_type=="primary" for x in history) else None,"binding_history":[{**binding_view(x),"room_name":rooms.get(x.room_id,""),"account_name":accounts.get(x.account_id,""),"anchor_name":anchors.get(x.anchor_id,"")} for x in history],"metrics_60m":[{"received_at":x.received_at,"cpu_percent":x.cpu_percent,"memory_percent":x.memory_percent,"network_latency_ms":x.network_latency_ms,"upload_mbps":x.upload_mbps,"stream_bitrate_kbps":x.stream_bitrate_kbps,"dropped_frames":x.dropped_frames} for x in metrics],"audit":[{"id":x.id,"actor":x.actor,"action":x.action,"target_type":x.target_type,"target_id":x.target_id,"details":x.details,"created_at":x.created_at} for x in logs]}

@router.put("/organization/devices/{device_id}/binding")
def organization_put_binding(device_id:str,payload:BindingPutIn,ctx:TenantContext=Depends(current_member),db:Session=Depends(get_db))->dict:
 authorize_feature_action(ctx,db,"device_binding","update","binding.write")
 try:
  row,idempotent=create_binding_transaction(db,device_id=device_id,organization_id=ctx.organization_id,member_id=ctx.member_id,room_id=payload.room_id,account_id=payload.account_id,anchor_id=payload.anchor_id,binding_type=payload.binding_type,reason=payload.reason)
  if not idempotent:audit_v2(db,ctx,"bind_device_room","device_room_binding",str(row.id),json.dumps({"device_id":device_id,"room_id":payload.room_id,"reason":payload.reason},ensure_ascii=False))
  db.commit();return {"binding":binding_view(row),"idempotent":idempotent}
 except (IntegrityError,OperationalError):
  db.rollback();raise api_error("BINDING_CONFLICT","设备或直播间存在冲突的主绑定") from None

@router.delete("/organization/devices/{device_id}/binding")
def organization_delete_binding(device_id:str,payload:BindingDeleteIn,ctx:TenantContext=Depends(current_member),db:Session=Depends(get_db))->dict:
 authorize_feature_action(ctx,db,"device_binding","delete","binding.write")
 try:row=end_primary_binding_transaction(db,device_id=device_id,organization_id=ctx.organization_id,reason=payload.reason)
 except OperationalError:
  db.rollback();raise api_error("BINDING_CONFLICT","设备绑定正在被其他请求修改") from None
 audit_v2(db,ctx,"unbind_device_room","device_room_binding",str(row.id),json.dumps({"device_id":device_id,"reason":payload.reason},ensure_ascii=False));db.commit();return {"ok":True,"binding":binding_view(row)}
...
def organization_settings(ctx:TenantContext=Depends(current_member),db:Session=Depends(get_db))->dict:
 require_permission(ctx,"organization.read");org=organization(db,ctx.organization_id);runtime=client_runtime_config(db)
 return {"organization":{"id":org.id,"name":org.name,"short_name":org.short_name,"contact":org.contact,"timezone":org.timezone,"status":org.status,"plan_code":org.plan_code},"subscription":{"starts_at":org.subscription_starts_at,"expires_at":org.subscription_expires_at,"grace_ends_at":org.grace_ends_at},"client_config":runtime,"config_version":effective_config_version(db,org)}

@router.put("/organization/settings")
def update_organization_settings(payload:OrganizationProfileIn,ctx:TenantContext=Depends(current_member),db:Session=Depends(get_db))->dict:
 authorize_feature_action(ctx,db,"member_management","manage","members.manage");org=organization(db,ctx.organization_id)
 before={"name":org.name,"short_name":org.short_name,"contact":org.contact,"timezone":org.timezone}
 org.name=payload.name;org.short_name=payload.short_name;org.contact=payload.contact;org.timezone=payload.timezone;org.feature_config_version+=1
 audit_v2(db,ctx,"update_organization_settings","customer",str(org.id),json.dumps({"before":before,"after":payload.model_dump()},ensure_ascii=False));db.commit()
 return {"ok":True,"config_version":effective_config_version(db,org)}

@router.get("/organization/audit")
def organization_audit(limit:int=Query(50,ge=1,le=200),offset:int=Query(0,ge=0),ctx:TenantContext=Depends(current_member),db:Session=Depends(get_db))->dict:
 require_permission(ctx,"organization.read");stmt=select(AuditLog).where(AuditLog.actor.like(f"org:{ctx.organization_id}:%"));total=db.scalar(select(func.count()).select_from(stmt.subquery())) or 0;rows=db.scalars(stmt.order_by(AuditLog.id.desc()).offset(offset).limit(limit)).all()
 return {"items":[{"id":x.id,"actor":x.actor,"action":x.action,"target_type":x.target_type,"target_id":x.target_id,"details":x.details,"created_at":x.created_at} for x in rows],"pagination":{"limit":limit,"offset":offset,"total":total}}

@router.get("/organization/reports")
def organization_reports(q:str="",limit:int=Query(100,ge=1,le=500),offset:int=Query(0,ge=0),ctx:TenantContext=Depends(current_member),db:Session=Depends(get_db))->dict:
 authorize_feature_action(ctx,db,"report_center","read","organization.read")
 stmt=select(CheckReport).join(Device,Device.device_id==CheckReport.device_id).where(Device.customer_id==ctx.organization_id)
 if q.strip():stmt=stmt.where(or_(CheckReport.device_id.contains(q.strip()),CheckReport.conclusion.contains(q.strip()),CheckReport.target_region_id.contains(q.strip())))
 total=db.scalar(select(func.count()).select_from(stmt.subquery())) or 0;rows=db.scalars(stmt.order_by(CheckReport.checked_at.desc()).offset(offset).limit(limit)).all()
 return {"items":[{"report_id":x.report_id,"device_id":x.device_id,"checked_at":x.checked_at,"readiness_level":x.readiness_level,"overall_status":x.overall_status,"conclusion":x.conclusion,"target_region_id":x.target_region_id,"environment_summary":x.environment_summary,"network_summary":x.network_summary,"hardware_summary":x.hardware_summary} for x in rows],"pagination":{"limit":limit,"offset":offset,"total":total}}

@router.get("/organization/reports/{report_id}")
def organization_report_detail(report_id:str,ctx:TenantContext=Depends(current_member),db:Session=Depends(get_db))->dict:
 authorize_feature_action(ctx,db,"report_center","read","organization.read");row=db.scalar(select(CheckReport).join(Device,Device.device_id==CheckReport.device_id).where(CheckReport.report_id==report_id,Device.customer_id==ctx.organization_id))
 if not row:raise api_error("TENANT_RESOURCE_NOT_FOUND","检测报告不存在或属于其他企业")
...
@router.delete("/platform/plans/{plan_id}/features/{feature_code}")
def clear_plan_feature(plan_id:str,feature_code:str,admin:dict=Depends(require_platform_csrf),db:Session=Depends(get_db))->dict:
 feature=_feature_by_code(db,feature_code);row=db.scalar(select(PlanFeature).where(PlanFeature.plan_id==plan_id,PlanFeature.feature_id==feature.id))
 if not row:raise api_error("TENANT_RESOURCE_NOT_FOUND","套餐没有该功能覆盖")
 db.delete(row);bump_feature_config_epoch(db);db.add(AuditLog(actor=admin["username"],action="clear_plan_feature",target_type="subscription_plan",target_id=plan_id,details=json.dumps({"feature_code":feature_code},ensure_ascii=False)));db.commit();return {"ok":True}
...
def platform_organization_features(organization_id:int,admin:dict=Depends(current_platform_admin),db:Session=Depends(get_db))->dict:
 org=organization(db,organization_id);features=FeatureService(db,organization_id).get_effective_features_for_organization();definitions={x.id:x.feature_code for x in db.scalars(select(FeatureDefinition)).all()};overrides={definitions[x.feature_id]:x for x in db.scalars(select(OrganizationFeature).where(OrganizationFeature.organization_id==organization_id)).all() if x.feature_id in definitions}
 for code,item in features.items():
  row=overrides.get(code);item["override"]={"enabled":row.enabled,"source":row.source,"config":json.loads(row.config_json or "{}"),"starts_at":row.starts_at,"expires_at":row.expires_at} if row else None
 return {"organization":{"id":org.id,"name":org.name,"plan_code":org.plan_code},"features":features,"config_version":effective_config_version(db,org)}
...
@router.delete("/platform/organizations/{organization_id}/features/{feature_code}")
def clear_organization_feature(organization_id:int,feature_code:str,admin:dict=Depends(require_platform_csrf),db:Session=Depends(get_db))->dict:
 org=organization(db,organization_id);feature=_feature_by_code(db,feature_code);row=db.scalar(select(OrganizationFeature).where(OrganizationFeature.organization_id==organization_id,OrganizationFeature.feature_id==feature.id))
 if not row:raise api_error("TENANT_RESOURCE_NOT_FOUND","企业没有该功能覆盖")
 before={"feature_code":feature_code,"enabled":row.enabled,"source":row.source};db.delete(row);org.feature_config_version+=1;db.add(AuditLog(actor=admin["username"],action="clear_organization_feature",target_type="customer",target_id=str(org.id),details=json.dumps(before,ensure_ascii=False)));db.commit();return {"ok":True,"config_version":effective_config_version(db,org)}
...
def platform_device_features(device_id:str,admin:dict=Depends(current_platform_admin),db:Session=Depends(get_db))->dict:
 ...
 org=organization(db,device.customer_id);features=FeatureService(db,device.customer_id).get_effective_features_for_device(device,device.app_version);definitions={x.id:x.feature_code for x in db.scalars(select(FeatureDefinition)).all()};overrides={definitions[x.feature_id]:x for x in db.scalars(select(DeviceFeatureOverride).where(DeviceFeatureOverride.device_id==device_id)).all() if x.feature_id in definitions}
 for code,item in features.items():
  row=overrides.get(code);item["override"]={"enabled":row.enabled,"config":json.loads(row.config_json or "{}"),"reason":row.reason,"expires_at":row.expires_at} if row else None
 return {"device":{"device_id":device.device_id,"display_name":device.display_name or device.device_id,"organization_id":device.customer_id,"agent_version":device.app_version},"features":features,"config_version":effective_config_version(db,org,device)}
...
@router.delete("/platform/devices/{device_id}/features/{feature_code}")
def clear_device_feature(device_id:str,feature_code:str,admin:dict=Depends(require_platform_csrf),db:Session=Depends(get_db))->dict:
 device=db.get(Device,device_id)
 if not device or not device.customer_id:raise api_error("TENANT_RESOURCE_NOT_FOUND","设备不存在或尚未加入企业")
 feature=_feature_by_code(db,feature_code);row=db.scalar(select(DeviceFeatureOverride).where(DeviceFeatureOverride.device_id==device_id,DeviceFeatureOverride.feature_id==feature.id))
 if not row:raise api_error("TENANT_RESOURCE_NOT_FOUND","设备没有该功能覆盖")
 db.delete(row);device.feature_config_version+=1;db.add(AuditLog(actor=admin["username"],action="clear_device_feature",target_type="device",target_id=device_id,details=json.dumps({"feature_code":feature_code},ensure_ascii=False)));db.commit();org=organization(db,device.customer_id);return {"ok":True,"config_version":effective_config_version(db,org,device)}
...
def platform_role_features(organization_id:int,role:str,admin:dict=Depends(current_platform_admin),db:Session=Depends(get_db))->dict:
 ...
 service=FeatureService(db,organization_id);features=service.get_effective_features_for_organization(role);base=service.get_effective_features_for_organization("owner")
 return {"role":role,"base_permissions":sorted(ROLE_PERMISSIONS[role]),"items":[{"feature_code":code,"feature_name":item["name"],"category":item["category"],"feature_enabled":base.get(code,{}).get("enabled",False),**item["permissions"]} for code,item in features.items()]}
...
@router.delete("/platform/organizations/{organization_id}/roles/{role}/features")
def reset_role_features(organization_id:int,role:str,admin:dict=Depends(require_platform_csrf),db:Session=Depends(get_db))->dict:
 if role not in ROLE_PERMISSIONS:raise api_error("VALIDATION_ERROR","角色不存在")
 org=organization(db,organization_id);rows=db.scalars(select(RoleFeaturePermission).where(RoleFeaturePermission.organization_id==organization_id,RoleFeaturePermission.role==role)).all()
 for row in rows:db.delete(row)
 org.feature_config_version+=1;db.add(AuditLog(actor=admin["username"],action="reset_role_features",target_type="organization_role",target_id=f"{organization_id}:{role}",details=json.dumps({"deleted":len(rows)},ensure_ascii=False)));db.commit();return {"ok":True,"config_version":effective_config_version(db,org)}
...
@router.delete("/platform/features/{feature_id}/rollouts/{rollout_id}")
def delete_feature_rollout(feature_id:int,rollout_id:int,admin:dict=Depends(require_platform_csrf),db:Session=Depends(get_db))->dict:
 row=db.scalar(select(FeatureRollout).where(FeatureRollout.id==rollout_id,FeatureRollout.feature_id==feature_id))
 if not row:raise api_error("FEATURE_NOT_FOUND","灰度规则不存在")
 before=_rollout_view(row);db.delete(row);bump_feature_config_epoch(db);db.add(AuditLog(actor=admin["username"],action="delete_rollout",target_type="feature_rollout",target_id=str(rollout_id),details=json.dumps(before,ensure_ascii=False,default=str)));db.commit();return {"ok":True}
